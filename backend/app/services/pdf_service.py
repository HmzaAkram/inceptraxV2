...
class PDFService:
    ...
    def generate_report(idea):
        """Generates a PDF report for an idea and returns the file path."""
        # Setup paths
        ...
        # If exists, we could return it, but user asked for "latest data" or "reuse if exists"
        # Since analysis_data is effectively immutable once generated (unless reanalyzed),
        ...
        elements.append(Paragraph(f"Full Business Analysis Report", styles['Heading2']))
        ...
        if not analysis:
            ...
            elements.append(Paragraph(f"<b>Overall Score: {analysis.get('overall_score', 'N/A')}/100</b>", body_style))
            elements.append(Spacer(1, 6))
            elements.append(Paragraph(f"<b>Market Demand:</b> {analysis.get('market_demand', 'N/A')}", body_style))
            elements.append(Paragraph(f"<b>Growth Potential:</b> {analysis.get('growth_potential', 'N/A')}", body_style))
            ...
            elements.append(Paragraph("Critical Risks", styles['Heading3']))
            ...
            for comp in analysis.get('competitors', []):
                elements.append(Paragraph(f"<b>{comp.get('name')}</b> ({comp.get('type')})", body_style))
